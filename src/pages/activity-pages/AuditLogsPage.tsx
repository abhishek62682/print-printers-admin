...
import { getAllAuditLogs } from '@/config/api/auditLog.api';
import type { AuditLog, GetAllLogsParams } from '@/config/api/auditLog.api';
...
const AllAuditLogsPage = () => {
    ...
    const [action, setAction] = useState<GetAllLogsParams['action'] | 'all'>('all');
    const [status, setStatus] = useState<GetAllLogsParams['status'] | 'all'>('all');
    const [search, setSearch] = useState('');

    const queryParams: GetAllLogsParams = {
        page,
        limit,
        ...(action !== 'all' ? { action } : {}),
        ...(status !== 'all' ? { status } : {}),
    ...
    const { data, isLoading, isError } = useQuery({
        queryKey: ['all-audit-logs', queryParams],
        queryFn: () => getAllAuditLogs(queryParams),
        staleTime: 10000,
    });
    ...
    const handleActionChange = (val: string) => {
        setAction(val as GetAllLogsParams['action'] | 'all');
        setPage(1);
    };

    const handleStatusChange = (val: string) => {
        setStatus(val as GetAllLogsParams['status'] | 'all');
        setPage(1);
    };
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {logs?.map((log: AuditLog) => {
                                        const user = typeof log?.userId === 'string' ? null : log?.userId;
                                        return (
